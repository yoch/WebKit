// Topology G: scorer is only invoked through a wrapper (call-linking / inlining surface).
...
function wrapper(results, docId, termFreq) {
    return scorePostingDoc(results, docId, termFreq);
}

var results = new Map();
for (var i = 0; i < INSERTS; ++i)
    wrapper(results, i, 1 + (i & 3));
var start = nowMs();
for (var i = 0; i < UPDATES; ++i)
    wrapper(results, i % INSERTS, 1 + (i & 3));
if (results.size !== INSERTS)
    throw new Error("unexpected result size: " + results.size);
out("REPRO name=topo-wrapper-call elapsed_ms=" + (nowMs() - start).toFixed(6) + " sink=" + scorePostingDoc.state.sink + " size=" + results.size);
